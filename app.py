...
# Renderiza o script de forma invisível na página
...
if categoria == "Financeiras":
    ...
    with aba_fin[0]:
        ...
        aporte = st.number_input("Aportes Mensais (R$)", value=200.0, key="j_aporte")
    ...
    with aba_fin[1]:
        ...
        cotacao_usd = st.number_input("Cotação do Dólar (R$)", value=5.0, key="m_usd")
    ...
    with aba_fin[3]:
        ...
        if st.button("Calcular IRPF", key="b_irpf"):
            ...
            st.success(
                f"Alíquota efetiva: {aliquota} | Imposto Retido Estimado: R$ {imposto:.2f}"
            )
    ...
    with aba_fin[5]:
        ...
        if st.button("Calcular Parcelas", key="b_emp"):
            ...
            st.success(
                f"Valor da Parcela Mensal: R$ {pmt:.2f} | Total pago: R$ {pmt * meses_emp:.2f}"
            )

    with aba_fin[6]:
        st.subheader("Simulador de Financiamento Imobiliário")
        imovel = st.number_input("Valor do Imóvel (R$)", value=300000.0, key="fin_im")
        entrada = st.number_input("Valor da Entrada (R$)", value=60000.0, key="fin_ent")
        anos = st.number_input("Prazo (anos)", value=30, key="fin_ano")
        taxa_anual = st.number_input("Taxa de Juros Anual (%)", value=10.0, key="fin_tx")
        if st.button("Calcular Financiamento", key="b_fin"):
            ...
            st.success(
                f"Valor financiado: R$ {valor_fin:.2f} | 1ª Parcela estimada: R$ {pmt_im:.2f}"
            )

    with aba_fin[7]:
        st.subheader("Cálculo Estimado de Férias")
        sal_ferias = st.number_input("Salário Bruto Mensal (R$)", value=3000.0, key="f_sal")
        dias_ferias = st.number_input("Dias de Férias a gozar", value=30, key="f_dias")
        if st.button("Calcular Férias", key="b_ferias"):
            ...
            st.success(
                f"Valor Bruto das Férias (com 1/3 constitucional): R$ {total_ferias:.2f}"
            )
...
# --- 2. CATEGORIA: SAÚDE E BEM-ESTAR ---
    ...
    with aba_saude[1]:
        ...
        if st.button("Calcular Gasto Total", key="b_tmb"):
            ...
            st.success(
                f"Gasto Basal: {tmb:.1f} kcal | Gasto Total Diário: **{gasto_total:.1f} kcal**"
            )
    ...
    with aba_saude[2]:
        ...
        if st.button("Calcular Água", key="b_agua"):
            ...
            st.success(
                f"Você deve consumir aproximadamente **{(total_ml / 1000):.2f} litros** de água por dia."
            )
    ...
    with aba_saude[3]:
        ...
        if st.button("Calcular Frequência", key="b_fc"):
            fc_max = 220 - idade_fc
            st.success(
                f"Frequência Cardíaca Máxima: {fc_max} bpm (Ideal para treinos: {int(fc_max * 0.6)} a {int(fc_max * 0.85)} bpm)"
            )
...
# --- 3. CATEGORIA: MATEMÁTICA E PESSOAL ---
    ...
    with aba_mat[6]:
        ...
        if st.button("Converter Base", key="b_base"):
            st.success(
                f"Binário: {bin(int(dec_val))} | Hexadecimal: {hex(int(dec_val)).upper()}"
            )
...
# --- 4. CATEGORIA: UTILIDADES E DIA A DIA ---
    ...
    with aba_util[0]:
        ...
        if st.button("Calcular Suprimentos", key="b_churras"):
            ...
            st.success(
                f"🥩 Carne necessária: {carne:.1f} kg | 🍺 Cerveja: {cerveja} latas | 🥤 Refrigerante: {refrigerante:.1f} litros"
            )
    ...
    with aba_util[4]:
        ...
        if st.button("Calcular Total", key="b_gorg"):
            ...
            st.success(
                f"Gorjeta: R$ {gorjeta:.2f} | Total com gorjeta: R$ {conta + gorjeta:.2f}"
            )

    with aba_util[5]:
        st.subheader("Custo de Combustível por Viagem")
        distancia_v = st.number_input("Distância da viagem (km)", value=250.0, key="cb_d")
        consumo_carro = st.number_input("Consumo médio do carro (km/l)", value=12.0, key="cb_c")
        preco_comb = st.number_input("Preço do litro do combustível (R$)", value=5.89, key="cb_p")
        if st.button("Calcular Custo", key="b_cb"):
            litros = distancia_v / consumo_carro
            custo_total = litros * preco_comb
            st.success(
                f"Você vai gastar ~{litros:.1f} litros, totalizando R$ {custo_total:.2f}"
            )
    ...
    with aba_util[7]:
        ...
        if st.button("Converter para MB", key="b_stor"):
            st.success(
                f"{val_gb} GB equivalem a {val_gb * 1024:.0f} MB ou {val_gb / 1024:.2f} TB"
            )
